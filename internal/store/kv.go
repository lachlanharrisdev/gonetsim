// Package store provides a small budgeted string-to-string key/value store.
// Scripts use it for state scoped to a connection, a handler, or the whole
// process. All limits are enforced at write time so one runaway handler
...
// Budget tracks how much of the shared state budget a set of stores has used.
type Budget struct {
	mu    sync.RWMutex
	limit int64
	used  int64
...
// Store is a bounded string-to-string map.
type Store struct {
	budget *Budget
