// Package tlscert provides TLS server certificates for listeners: explicit
// cert/key pairs, an ephemeral in-memory self-signed cert, or a persisted
// self-signed pair plus a CA export. Persistence matters in air-gapped labs
// where a host needs to trust the simulator once and keep it stable across
// restarts.
package tlscert
...
// Config selects how a listener gets its certificate. When CertFile is empty
// an ephemeral self-signed certificate is generated in memory for the run.
type Config struct {
	CertFile string
	KeyFile  string

	// controls the fallback self-signed cert generation (DNS names/IPs).
...
// Regenerate removes a persisted auto-pair so the next TLSConfig() call
// creates a fresh one.
func (c Config) Regenerate() error {
	if !isPersistedAutoPair(c.CertFile, c.KeyFile) {
		return errors.New("regenerate only applies to the gonetsim-cert.pem/key.pem pair")
	}
